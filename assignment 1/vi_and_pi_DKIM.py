...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    prob, s_next, r = as_tensor(P, nS, nA)
    # nS, nA, nS 
    num = np.arange(nS)
    while True:
        #??? : value_function = np.sum(value_function + (r + prob * gamma * value_function[s_next]),axis =2)
        temp = prob * (r + gamma * value_function[s_next])
        temp = np.sum(temp,axis =2)
        value_function = temp[num, policy[num]]
        if abs(value_function - value_function_prev).max() < tol:
            break
        value_function_prev = value_function    
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    temp = prob * (r + gamma * value_from_policy[s_next])
    temp = np.sum(temp,axis =2)
    new_policy = np.argmax(temp, axis = 1)
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    while True:
        ...
        policy = new_policy  
    ############################
    print("policy iteration optimal policy : ", policy)
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        new_value_function = np.maximum(value_function, temp)
        if abs(new_value_function - value_function).max() < tol:
            break
        value_function = new_value_function
    
    print("value iteration optimal policy : ", policy)
    ############################
    return value_function, policy
